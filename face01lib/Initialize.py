...
class Initialize:
    ...
    def initialize(self, conf_dict):
        ...
        # kaoninshoDir: str = 
        known_face_encodings, known_face_names = load_priset_image(self, self.conf_dict["kaoninshoDir"],self.conf_dict["priset_face_imagesDir"])

        # set_width,fps,height,width,set_height
        set_width,fps,height,width,set_height = \
            VidCap().return_movie_property(self.conf_dict["set_width"], VidCap().return_vcap(self.conf_dict["movie"]))
