...
class LibDraw:
    ...
    # target_rectangleの描画
    def draw_target_rectangle(self, rect01_png,resized_frame,top,bottom,left,right,name):
        ...
        if not self.name == 'Unknown':  ## self.nameが既知の場合
            ...
            x1, y1, x2, y2 = self.left, self.top, self.left + rect01_NG_png.shape[1], self.top + rect01_NG_png.shape[0]
            try:
                self.resized_frame[y1:y2, x1:x2] = self.resized_frame[y1:y2, x1:x2] * (1 - rect01_NG_png[:,:,3:] / 255) + \
