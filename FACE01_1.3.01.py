...
# from face01lib129.cal_angle_coordinate import cal_angle_coordinate
from face01lib129.load_priset_image import load_priset_image
from face01lib129.return_percentage import return_percentage
from face01lib129.similar_percentage_to_tolerance import to_tolerance
from face01lib129.video_capture import video_capture

"""使用しないCythonファイル
# from face01lib129.draw_telop import draw_telop
# from face01lib129.return_frame_coordinate import return_frame_coordinate
"""
        ...
        if len(face_encodings) > 0:  # ↑この行は上記処理の代わり。この行を入れないと以降の処理行すべてインデントし直さないといけないため。
            ...
            for (top, right, bottom, left), name in zip(face_location_list, face_names):
                ...
                if name == 'Unknown':
                    ...
                    return_percentage(p)
                    """
                    percentage = return_percentage(p)  # cython版
